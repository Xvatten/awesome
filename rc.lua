...
run_once("xrandr --output DVI-I-1 --mode 1920x1080 --rate 144")
run_once("roccateventhandler")
...
-- beautiful init
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/shotgun/theme.lua")
...
-- {{{ Tags
tags = {
   names = { "web", "term", "games", "chat", "steam", "media", "video", "other" }, 
   layout = { layouts[3], layouts[2], layouts[3], layouts[6], layouts[1], layouts[6], layouts[6], layouts[2] }
}
...
globalkeys = awful.util.table.join(
    ...
    -- Dropdown terminal
    awful.key({ modkey,	          }, "F1",      function () drop(terminal) end),
    ...
    awful.key({}, "XF86Sleep", 
    	function () 
    os.execute(string.format("i3lock -i /home/shotgun/.config/awesome/themes/shotgun/wall.png -c 000000")) 
    end),
    awful.key({}, "XF86Print", 
    	function () 
    os.execute(string.format("scrot /home/shotgun/Pictures/AwesomeScreenshots/X30_%d_%m_%y_%H_%M_%S.png")) 
    end),
...
awful.rules.rules = {
    ...
    { rule = { class = "ShadowOfMordor" },
          properties = { tag = tags[1][3] } },

    { rule = { class = "Render" },
          properties = { tag = tags[1][3] } },

    { rule = { class = "DyingLightGame" },
          properties = { tag = tags[1][3] } },
